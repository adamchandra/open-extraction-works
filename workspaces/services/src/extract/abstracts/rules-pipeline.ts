...
import { findByLineMatchTE, findInMetaTE, selectElemAttr } from '~/extract/core/field-extract-utils';
...
export const AbstractPipeline: ExtractionFunction[][] = [
  ...
  [filterUrl(/ijcai.org\/Abstract/), findByLineMatchTE(['|', 'p', '|'], { lineOffset: -1, lineCount: 1 })],
  [filterUrl(/etheses.whiterose.ac.uk/), findByLineMatchTE(['h2', '| Abstract'], { lineOffset: -1 })],
  [filterUrl(/ndss-symposium.org\/ndss-paper/), findByLineMatchTE([' +|', ' +p', ' +p', ' +|'], { lineOffset: 1 })],
  [
    filterUrl(/openreview.net/),
    findByLineMatchTE(['.note-content-field', '| Abstract', '.note-content-value'], { lineOffset: 2 })
  ],
  [filterUrl(/ieee.org/), findInGlobalDocumentMetadata],
  ...
  [findByLineMatchTE(['div.*#abstract', 'h4', 'Abstract'], { evidenceEnd: ['div.*#paperSubject', 'h4', 'Keywords'] })],

  [findByLineMatchTE(['div', 'h4', '| Abstract', 'p'], { evidenceEnd: ['div'] })],
  // 23.
  ...
  [findByLineMatchTE(['.cPageSubtitle', '| Abstract'], { evidenceEnd: ['.cPageSubtitle', '| \\w'] })],

  [findByLineMatchTE(['^ +p', '^ +b', '^ +| Abstract:'], { indentOffset: -2, evidenceEnd: ['^ +p', '^ +b'] })],

  [findByLineMatchTE(['^ +i', '^ +b', '^ +| Abstract:'], { indentOffset: -4, evidenceEnd: ['^ +p'] })],
